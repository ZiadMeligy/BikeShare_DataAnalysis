...
def get_filters():
    ...
    global cityglobal # making a global variable to use in user_stats. check the function to see how that variable is used.
    cityglobal = city
    print('-'*40)
...
def user_stats(df):
    ...
    if cityglobal=="washington":
        return print("Washington Data do not have Gender and birth year statistics!") # leaves the function if city is washington
...
def main():
    while True:
        ...
        while QA =="yes":
            print(df.iloc[stiloc:endiloc]) #shows rows from stiiloc to endiloc
